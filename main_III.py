...
st.title('Tracking by using Streamlit')
...
st.markdown(
""" The date available for 2019/01/01 to 2019/01/05, if you do not input
    number, it'll show table for 2019/01/01 data as default
""")

df1 = pd.read_csv('https://raw.githubusercontent.com/SiwakornPr/Tracking/master/20190101.csv')[:15000]
date = st.text_input(label='Date Example: 2019/01/03')
if date == '' or '1':
    df = df1
if date == '2':
    df = pd.read_csv('https://raw.githubusercontent.com/SiwakornPr/Tracking/master/20190102.csv')[:15000]
if date == '3':
    df = pd.read_csv('https://raw.githubusercontent.com/SiwakornPr/Tracking/master/20190103.csv')[:15000]
if date == '4':
    df = pd.read_csv('https://raw.githubusercontent.com/SiwakornPr/Tracking/master/20190104.csv')[:15000]
if date =='5':
    df = pd.read_csv('https://raw.githubusercontent.com/SiwakornPr/Tracking/master/20190105.csv')[:15000]
...
ID_Q = st.text_input(label='ID Example: 1, Note: The demo only contain 15,000 of data per day')
st.markdown(
...
""" Note: Some point, picked up location and deliveried location
    are the same location then it'll appear only one point. (In case of cancel trip)
""") #Such as ID: 900
...
def ID_Query(latitudes, longitudes, t, color):
    for lat, lng, ID, start in zip(latitudes, longitudes, labels, t):
        if ID_Q == str(ID):
            fo.Marker(
                location = [lat, lng],
                popup = ['ID: ' + str(ID), 'latitude: ' + str(lat)[:8], 'longitude: ' + str(lng)[:9], start],
...
hour_set = st.slider("Hour to look at (Every 1 hour)", 0, 23, step = 1, key = "geo_3d")
...
st.subheader("Geo data (Picked up) at %i:00" % (hour_set) + "to %i:59" % (hour_set))
geo_3d('lonstartl', 'latstartl')
st.subheader("Geo data (Deliveried) at %i:00" % (hour_set) + "to %i:59" % (hour_set))
geo_3d('lonstop', 'latstop')

#hour_set_eng = st.slider("Hour to look at (Every 1 hour)", 0, 23, step = 1, key = "eng")
st.subheader("Engagement (picked up) by minute %i:00" % (hour_set))
def chart(time: str, type: str):
    filtered = data[data[time].dt.hour == hour_set]
        #[(data[time].dt.hour >= hour_set_eng) & (data[time].dt.hour < (hour_set_eng + 1))]
